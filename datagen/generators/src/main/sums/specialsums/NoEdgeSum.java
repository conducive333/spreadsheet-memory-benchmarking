...
public class NoEdgeSum extends BaseSpecialSum implements Creatable {
  /**
   ...
   *  MAX_V_ROWS  |   ?   |   =EVAL(?)  |
   * 
   * ? = a random value (or a placeholder value if no random 
   * seed is specified). The COLS parameter controls the number
   * of columns with values AND the number of columns with 
   * formulae. For example, if COLS = 2, then there will be 2 
   * columns of values and 2 columns of formulae. The window 
   * size adjusts the number of cells in the range. In the example
   * above, WINDOW_SZE = 2. EVAL(...) means the evaluated result 
   * of the input formula and N is the number of rows.
   */

  private static final String CREATE_STR = "SUM(%f)";

  public NoEdgeSum (int uppr) {
    super(uppr);
  }
    ...
    for (int r = 0; r < MAX_V_ROWS; r++) {
      ...
      for (int c = 0; c < cols; c++) {
        ...
        if (r < rows) {
          ...
          fRow.createCell(c + cols).setCellValue(FILL_VALUE);
        }
    ...
    for (int r = 0; r < MAX_V_ROWS; r++) {
      ...
      for (int c = 0; c < cols; c++) {
        ...
        if (r < rows) {
          ...
          fRow.createCell(c + cols).setCellValue(num);
        }
    ...
    for (int r = 0; r < MAX_V_ROWS; r++) {
      ...
      for (int c = 0; c < cols; c++) {
        ...
        if (r < rows) {
          ...
          fRow.getOrCreateCell(c + cols).setFormula(FILL_VALUE + "");          
        }
    ...
    for (int r = 0; r < MAX_V_ROWS; r++) {
      ...
      for (int c = 0; c < cols; c++) {
        ...
        if (r < rows) {
          ...
          fRow.getOrCreateCell(c + cols).setFormula(num + "");
        }
